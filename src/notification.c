                ...
                GError          **error)
{
  ...
  if (g_strv_contains (priorities, priority))
    ...
    g_debug ("Unsupported priority %s filtered from notification", priority);
              ...
              GError          **error)
{
  ...
  if (label && action)
    ...
    {
      g_debug ("Notification button needs a label and action property");
    }
...
static void
add_in_thread_finished_cb (GObject      *source_object,
                           GAsyncResult *result,
                           gpointer      user_data)
{
  CallData *call_data;
